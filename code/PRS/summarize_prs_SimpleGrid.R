...
df <- df %>% select(SCORE1_SUM, Pop, Lat, Long)

# Compute the average
dfOut <- df %>% mutate(nc_stan = (SCORE1_SUM - mean(SCORE1_SUM)) / sd(SCORE1_SUM)) %>% group_by(Pop) %>% summarise(avg_PRS = mean(nc_stan), LAT = mean(Lat), LONG = mean(Long))
