...
```{r}
df <- fread("../plots/SimpleGrid/A/q_PS.txt")[,3:19]
...
```{r}
df <- fread("../plots/SimpleGrid/A/tildeError_PS.txt")[,2:10]

# Average Bias and false positive rate 
dfPlot <- df %>% group_by(gwas_size, test_size, L, test) %>% summarize(count = n(), avgFGr = mean(FGr), avg10 = mean(`10`), avg35 = mean(`35`), avg100 = mean(`100`)) %>% select()
