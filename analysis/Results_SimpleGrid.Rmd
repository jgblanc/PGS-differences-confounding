...
# Order the types
dfPlot$type <- factor(dfPlot$type, levels = c("NoCorrection","LMMNoCorrection","10", "FGr" ))
...
dfPlot_Long$type <- factor(dfPlot_Long$type, levels = c("NoCorrection","LMMNoCorrection","10", "FGr" ))
...
plot_bias <- function(df, pheno_type, test_type, env_type) {
  ...
  scale_fill_manual(values = c("goldenrod2","turquoise4", "red4" , "navy")) + scale_x_discrete(labels = unname(TeX(c("Uncorrected", "LMM", "10 PCs", "$\\hat{F}_{Gr}$")))) + xlab("") + ylab("Bias") + facet_wrap(~test)
