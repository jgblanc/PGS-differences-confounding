...
plot_bias <- function(pheno_type, test_type, env_type) {
  
  pl <- dfPlot %>% filter(pheno == pheno_type & test == test_type & env == env_type & type != "ID") %>% ggplot(aes(x = type, y = avg_q, fill = type)) + geom_col()+ geom_point(fill = "black", size = 3) + geom_hline(yintercept = 0, color = "red") + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean), width = 0.2) +
  ...
  scale_fill_manual(values = c("goldenrod2","navy", "red4")) + scale_x_discrete(labels = unname(TeX(c("Uncorrected", "$\\hat{F}_{Gr}$", "10 PCs")))) + xlab("") + ylab("Bias")
  
  return(pl)
}

# Plot bias 
plLL <- plot_bias("LAT", "LAT", 0.2)
plDL <- plot_bias("DIAG", "LAT", 0.2)
plPS <- plot_bias("PS", "PS", 0.2)
```
