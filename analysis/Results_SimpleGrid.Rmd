...
# Read in results 
df <- fread("../plots/SimpleGrid/A/EqualSS/q.txt")
df <- df [,3:ncol(df)]
...
prsDL <- plot_PRS(dfNC, dfFGr, dfPCs, -.22, .4)
prsDL
...
prsPS <- plot_PRS(dfNC, dfFGr, dfPCs, -.12, .12)
prsPS
```
...
# Lat
df <- fread("../plots/SimpleGrid/gp-1440/tp-1440/h2-0.0/c-300/env_0.2/LAT/phenos_L-20000.txt")
phenoLat <- plot_pheno(df)
phenoLat

# Diag
df <- fread("../plots/SimpleGrid/gp-1440/tp-1440/h2-0.0/c-300/env_0.2/DIAG/phenos_L-20000.txt")
phenoDiag <- plot_pheno(df)
phenoDiag

# PS 
df <- fread("../plots/SimpleGrid/gp-1440/tp-1440/h2-0.0/c-300/env_0.2/PS/phenos_L-20000.txt")
...
df <- fread("../plots/SimpleGrid/A/q_PS2.txt")[,3:19]

# Average Bias and false positive rate 
dfPlot <- df %>% group_by(gwas_size, test_size, h2, num_causal, env, L, type, pheno, test) %>% summarize(count = n(), avg_q = mean(`nc-q`),lowerci_mean = avg_q - (1.96 * (sd(`nc-q`)  /  sqrt(n()))), upperci_mean = avg_q + (1.96 * (sd(`nc-q`)  /  sqrt(n()))), fp_strat = sum(`nc-p` < 0.05)/ n(), lowerci_fp = fp_strat - 1.96 * sqrt((fp_strat * (1-fp_strat))/ n()), upperci_fp = fp_strat + 1.96 * sqrt((fp_strat * (1-fp_strat))/ n()), q_std = avg_q / sd(`nc-q`)) %>% filter(pheno == "PS" & test == "PS") 
...
# Plot 
pl <- ggplot(data = dfPlot, aes(x = L, y = avg_q, color = type)) + geom_point(size = 2.5, position=position_dodge(width=500)) + geom_hline(yintercept = 0, color = "red") + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean),  width = 900, position=position_dodge(width=500)) +  
  theme_bw(base_size = 14) + theme(legend.position = "bottom", panel.border = element_rect(colour = "black", fill=NA, size=1), legend.text = element_text(size = 10), panel.grid.minor = element_line(linetype = 'dashed'), strip.background =element_rect(fill="white")) + ylab("Bias") +
  scale_color_manual(values = c("goldenrod2", "red4", "darkorange3", "navy"), labels = c("No Correction","10 PCs", "35 PCs", TeX("$\\hat{F}_{Gr}$")), name = "") + facet_wrap(~TEST) + scale_x_continuous(breaks = seq(2000, 200000, by = 2000))
pl

ggsave("~/Desktop/PS_Bias.png", pl, width = 7, height = 5)
```

```{r}
df <- fread("../plots/SimpleGrid/A/tildeError_PS_v2.txt")[,2:10]

# Average Bias and false positive rate 
dfPlot <- df %>% group_by(gwas_size, test_size, L, test) %>% summarize(count = n(), avgFGr = mean(FGr), avg10 = mean(`10`), avg35 = mean(`35`), avg100 = mean(`100`))
#dfPlot <- df %>% filter(rep == "A1")

dfMelt <- dfPlot %>% pivot_longer(col = c(avgFGr, avg10, avg35, avg100))
#dfMelt <- dfPlot %>% pivot_longer(col = c(FGr, `10`, `35`, `100`))

# Plot 
ggplot(data = dfMelt, aes(x = L, y = value, color = name)) + geom_point(size = 3) + geom_hline(yintercept = 0, color = "red") + ylab("Error")
```

```{r}
test <- fread("~/Downloads/LOCO_FGr-L-20000.txt")
test2 <- fread("~/Downloads/genos.pop")
colnames(test2) <- c("FID", "IID", "POP", "LAT", "LONG")
df <- inner_join(test, test2)

ggplot(df, aes(x = LONG, y = LAT, fill = FGr)) + geom_tile() + theme_classic(base_size = 12) +  theme(panel.border = element_rect(colour = "black", fill=NA, size=1), legend.position = "none") + scale_fill_gradient2(low = "palegoldenrod", high = "purple4") + xlab("Longitude") + ylab("Latitude") + scale_y_continuous(breaks=seq(0,6,1)) + scale_x_continuous(breaks=seq(0,6,1)) + theme(axis.title = element_blank())

ggplot(df, aes(x = LONG, y = LAT, fill = tildeFGr)) + geom_tile() + theme_classic(base_size = 12) +  theme(panel.border = element_rect(colour = "black", fill=NA, size=1), legend.position = "none") + scale_fill_gradient2(low = "palegoldenrod", high = "purple4") + xlab("Longitude") + ylab("Latitude") + scale_y_continuous(breaks=seq(0,6,1)) + scale_x_continuous(breaks=seq(0,6,1)) + theme(axis.title = element_blank())
```
...
# Plot 
ggplot(data = dfPlot, aes(x = type, y = avg_q)) + geom_col()+ geom_point(fill = "black", size = 3) + geom_hline(yintercept = 0, color = "red") + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean), width = 0.2) + facet_wrap(~L)
```

## Bootstrap Error 

```{r}
df <- fread("../plots/SimpleGrid/A/error_PS.txt")[,2:7]

dfTable <- df %>% group_by(gwas_size, test_size, L, test) %>% summarise(count = n(), avg_error = mean(Error))
```
