...
# Plot 
ggplot(data = dfPlot, aes(x = L, y = avg_q, color = type)) + geom_point(size = 3) + geom_hline(yintercept = 0, color = "red") + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean)) 
```
...
```{r}
x <- scale(c(rep(-1, 10), rep(1, 10)))
y <- scale((x*0.5) + rnorm(20, 0, 0.01))
z <- (x*0.5) + rnorm(20, 0, 0.01)


mod1 <- lm(x ~ y)
mod2 <- lm(x ~ z)
r2_1 <- cor(x, fitted(mod1))^2
r2_2 <- cor(x, fitted(mod2))^2
...
norm.z <- norm(z,type="2")
one <- (dot.prod / (norm.x * norm.y))^2
two <- (dot.prod / (norm.x * norm.z))^2
