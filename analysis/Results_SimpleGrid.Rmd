...
plot_bias <- function(df, pheno_type, test_type, env_type) {
  ...
  pl <- df %>% filter(pheno == pheno_type & test == test_type & env == env_type & type != "ID") %>% ggplot(aes(x = type, y = avg_q, fill = type)) + geom_col()+ geom_point(fill = "black", size = 3) + geom_hline(yintercept = 0, color = "red") + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean), width = 0.2) +
