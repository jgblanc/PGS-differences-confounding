...
# Summarize data 
dfPlot_nc <- df %>% group_by(config, gwas_size, test_size, h2, ts, num_causal, env, L, type) %>% summarize(count = n(), avg_q = mean(`nc-q`),lowerci_mean = avg_q - (1.96 * (sd(`nc-q`)  /  sqrt(n()))), upperci_mean = avg_q + (1.96 * (sd(`nc-q`)  /  sqrt(n()))))
dfPlot_nc$effects <- "Ascertained" 
dfPlot_c <- df %>% group_by(config, gwas_size, test_size, h2, ts, num_causal, env, L, type) %>% summarize(count = n(), avg_q = mean(`c-q`),lowerci_mean = avg_q - (1.96 * (sd(`c-q`)  /  sqrt(n()))), upperci_mean = avg_q + (1.96 * (sd(`c-q`)  /  sqrt(n()))))
dfPlot_c$effects <- "Causal"
dfPlot <- rbind(dfPlot_nc, dfPlot_c)

# Get true q 
dfTrue <- df %>% group_by(config, gwas_size, test_size, h2, ts, num_causal, env, L) %>% summarize(count = n(), avg_q = mean(`true-q`))
...
# Plot data 
ggplot(data = dfFinal, aes(x = ts, y = avg_q, color = type)) + geom_point() + facet_grid(vars(env), vars(effects)) 
