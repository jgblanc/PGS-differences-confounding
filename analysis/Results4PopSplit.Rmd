...
```{r}
data <- inner_join(dfExp, dfPlot)

ggplot(data = data, aes(x = mean_exp_bias, y = avg_q, color = type)) + geom_point() + geom_abline(slope=1, intercept=0) + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean, color = type), width = 0.05) + xlab("Expected Bias") + ylab("Observed Bias")
```
