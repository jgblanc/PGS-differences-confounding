...
```{r}
df <- fread("../plots/4PopSplit/C/error.txt")
df$gamma <- df$L / 1000
...
df$gamma <- df$L / 1000
dfPlot <- df %>% group_by(config, test_size, gwas_size, L, type, gamma) %>% summarise(num = n(), avg_error = mean(error))

ggplot(data = dfPlot, aes(x = gamma, y = avg_error, color = type)) + geom_point() + scale_x_log10()
```

```{r}
df <- fread("../plots/4PopSplit/C/q.txt")
df <- df [,3:ncol(df)]
...
```{r}
tmp <- dfPlot %>% filter(env == 1)

ggplot(data = tmp, aes(x = gamma, y = fp_strat, color = type)) + geom_point() + scale_x_log10()
```
