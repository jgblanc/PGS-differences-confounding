...
```{r}
# Plot labels
labs <-  c("PC 1",TeX("$\\hat{F}_{Gr}$"), TeX("$\\tilde{F}_{Gr}$"), "No Correction")
...
# Plot
dfEB %>% filter(effects == "Causal") %>% ggplot(aes(x = x, y = avg_q, color = type))+ geom_point(position=position_dodge(width=1))+ facet_wrap(~facet2) + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean), width = 0.5, position=position_dodge(width=1)) + theme_classic(base_size = 12) + 
  theme(legend.text.align = 0) + theme(panel.border = element_rect(colour = "black", fill=NA, size=1), axis.title = element_text(size = 12)) +
  scale_colour_manual(name = "",labels = labs, values = c("red4" , "navy",  "darkgreen", "goldenrod2")) + 
  scale_shape_manual(name = "",labels = labs, values = c(16, 16, 16,16)) + 
  scale_size_manual(name = "",labels = labs,values = c(2, 2, 2, 2)) + geom_errorbar(data =dfTrue_c, aes(xmin = x, xmax = x, y = avg_q, yend = avg_q), color = "black", linetype = "dashed")

#+ geom_hline(data = dfTrue_c, aes(yintercept = avg_q), color = "black", linetype = "dashed")
```
