...
# Get true q 
dfTrue <- df%>% group_by(config, gwas_size, test_size, h2, ts, num_causal, env, L) %>% summarize(count = n())


# Plot data 
ggplot(data = dfPlot, aes(x = ts, y = avg_q, color = type)) + geom_point() + facet_wrap(~env) + geom_errorbar(aes(ymin = lowerci_mean, ymax = upperci_mean, color = type), width = 0.01)
